        ...
        if os.path.exists('%s.ini'%key):
            ...
            for table_name in table_list:
                ...
                for i in range(len(field_name)):
                    ...
                    root_value = jsonpath.jsonpath(value, "$..%s..%s" % (root, field_name[i]))
                    optionalRoots_request_value = jsonpath.jsonpath(value, "$..%s..%s" % (
                    optionalRoots_request, field_name[i]))
                    optionalRoots_response_value = jsonpath.jsonpath(value, "$..%s..%s" % (
                    optionalRoots_response, field_name[i]))
                    # print(root_value)
                    # 获取hive表的所有值
                    ...
                    if root_value != False:
                        ...
                        # print(queryTime,hive_value[0])
                        if queryTime in hive_value[0].encode("utf-8"):
                            print("%s字段效验通过" % field_name[i])
                        else:
                            print(field_name[i], queryTime, hive_value, hive_value[0].encode("utf-8"))
                    elif optionalRoots_request_value != False:
                    ...
                    elif optionalRoots_response_value != False:
                        ...
                        # if "None" in str(hive_value[0]):
                        #     print("{} 字段测试通过".format(field_name[i]))
                        # else:
                        print('{} 字段不满足上面的匹配条件,hive表内结果为{}'.format(field_name[i], hive_value))
            print("\n")
        else:
